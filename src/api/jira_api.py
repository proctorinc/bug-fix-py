...
def get_current_fix_version():
    ...
    response = requests.get(url, headers=constants.JIRA_REQUEST_HEADERS, auth=constants.JIRA_AUTH)
    json_response = json.loads(response.text)
...
def link_creation_chlc(chlrq, chlc):
    ...
    response = requests.post(url, json=body, headers=constants.JIRA_REQUEST_HEADERS, auth=constants.JIRA_AUTH)
